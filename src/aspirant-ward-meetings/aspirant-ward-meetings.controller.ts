...
export class AspirantWardMeetingsController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Aspirant: create a ward-level meeting for their ward' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: "Return active ward meetings for the authenticated aspirant's ward" })
  @ApiResponse({ status: 200, description: 'Meetings returned successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Aspirant profile not found for this user' })
  ...
  @ApiParam({ name: 'meetingId', type: 'number', description: 'Ward meeting ID' })
  @ApiResponse({ status: 200, description: 'Meeting marked completed' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden' })
  complete(@CurrentUser() user: any, @Param('meetingId') meetingId: string, @Body() dto: CompleteWardMeetingDto) {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Delete a ward meeting created for the aspirant\'s ward' })
  @ApiParam({ name: 'meetingId', type: 'number', description: 'Ward meeting ID' })
