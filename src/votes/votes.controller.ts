...
export class VotesController {
  ...
  @ApiResponse({ status: 400, description: 'Already voted or validation failed' })
  ...
  async results(@Param('wardId') wardId: number) {
    ...
    const total = rows.reduce((acc: number, r: any) => acc + Number(r.totalVotes || 0), 0);
  ...
  @Get('voting-window')
  @Public()
  @ApiOperation({ summary: 'Get the active voting window and check if voting is currently allowed' })
  @ApiResponse({ status: 200, description: 'Voting window information returned' })
