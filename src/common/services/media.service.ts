...
export class MediaService {
  constructor(
    ...
    @InjectRepository(AdminDocument) private readonly adminDocRepo: Repository<AdminDocument>,
  ...
  async deleteProfilePicture(userId: number): Promise<{ message: string }> {
    ...
    if (!user.profilePicture) throw new BadRequestException('No profile picture to delete');
  ...
  async uploadAspirantDocument(
    ...
    const url = await this.s3Service.uploadFile(file, `aspirants/${aspirantId}/documents`);
    ...
    switch (documentType) {
      case 'sop':
        ...
        break;
      case 'sop_kannada':
        if (aspirant.sopKannadaUrl) await this.s3Service.deleteFile(aspirant.sopKannadaUrl);
        aspirant.sopKannadaUrl = url;
        aspirant.sopKannadaStatus = 'pending';
        break;
      case 'agreement':
        if (aspirant.agreementUrl) await this.s3Service.deleteFile(aspirant.agreementUrl);
        aspirant.agreementUrl = url;
        aspirant.agreementStatus = 'pending';
        break;
      case 'property_declaration':
        if (aspirant.propertyDeclarationUrl) await this.s3Service.deleteFile(aspirant.propertyDeclarationUrl);
        aspirant.propertyDeclarationUrl = url;
        aspirant.propertyDeclarationStatus = 'pending';
        break;
      case 'code_of_conduct':
        if (aspirant.codeOfConductUrl) await this.s3Service.deleteFile(aspirant.codeOfConductUrl);
        aspirant.codeOfConductUrl = url;
        aspirant.codeOfConductStatus = 'pending';
        break;
      case 'resume':
        if (aspirant.resumeUrl) await this.s3Service.deleteFile(aspirant.resumeUrl);
        aspirant.resumeUrl = url;
        aspirant.resumeStatus = 'pending';
        break;
      case 'epic_card':
        if (aspirant.epicCardUrl) await this.s3Service.deleteFile(aspirant.epicCardUrl);
        aspirant.epicCardUrl = url;
        aspirant.epicCardStatus = 'pending';
        break;
      case 'epic_card_back':
        if (aspirant.epicCardBackUrl) await this.s3Service.deleteFile(aspirant.epicCardBackUrl);
        aspirant.epicCardBackUrl = url;
        aspirant.epicCardBackStatus = 'pending';
        break;
      case 'address_proof':
        if (aspirant.addressProofUrl) await this.s3Service.deleteFile(aspirant.addressProofUrl);
        aspirant.addressProofUrl = url;
        aspirant.addressProofStatus = 'pending';
        break;
      case 'recent_photo':
        if (aspirant.recentPhotoUrl) await this.s3Service.deleteFile(aspirant.recentPhotoUrl);
        aspirant.recentPhotoUrl = url;
        aspirant.recentPhotoStatus = 'pending';
        break;
      case 'selfie':
        if (aspirant.selfieUrl) await this.s3Service.deleteFile(aspirant.selfieUrl);
        aspirant.selfieUrl = url;
        aspirant.selfieStatus = 'pending';
        if (aspirant.userId) {
          const user = await this.userRepo.findOne({ where: { id: aspirant.userId } });
          if (user) {
            user.profilePicture = url;
            await this.userRepo.save(user);
          }
  ...
  async verifyAspirantDocument(
    ...
    if (verifyDto.status === 'rejected' && verifyDto.rejectionReason) {
      const reasons = aspirant.rejectionReasons ? JSON.parse(aspirant.rejectionReasons) : {};
    ...
    if (verifyDto.status === 'verified' && aspirant.rejectionReasons) {
      try {
        ...
        if (reasons && Object.prototype.hasOwnProperty.call(reasons, documentType)) {
          delete reasons[documentType];
          const keys = Object.keys(reasons || {});
          aspirant.rejectionReasons = keys.length ? JSON.stringify(reasons) : (null as any);
        }
      } catch (e) {
  ...
  async uploadAdminDocument(
    ...
    const url = await this.s3Service.uploadFile(file, `admin/documents/${documentType}`);
  ...
  async signDocument(
    ...
    const adminDoc = await this.adminDocRepo.findOne({ where: { id: adminDocumentId } });
    if (!adminDoc) {
      throw new NotFoundException('Admin document not found');
    }

    const url = await this.s3Service.uploadFile(file, `users/${userId}/signed-documents`);
  ...
  async verifyUserSignedDocument(
    ...
    const signedDoc = await this.userSignedDocRepo.findOne({ where: { id: signedDocId } });
    if (!signedDoc) {
