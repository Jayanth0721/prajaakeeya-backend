import { BadRequestException, ConflictException, ForbiddenException, Injectable, NotFoundException, OnModuleDestroy, OnModuleInit, UnauthorizedException } from '@nestjs/common';
...
export class AuthService implements OnModuleInit, OnModuleDestroy {
  ...
  async login(loginDto: LoginDto) {
    ...
    if (existing.role === 'aspirant') {
      throw new ForbiddenException('Aspirants must use mobile OTP login. Please use the aspirant login endpoint.');
    }
    ...
    return { token: await this.jwtService.signAsync(payload), user: userWithWard };
  }
  ...
  async adminLogin(loginDto: LoginDto) {
    ...
    const hash = crypto.scryptSync(loginDto.password, existing.passwordSalt, 64).toString('hex');
  ...
  async verifyOtp(verifyOtpDto: VerifyOtpDto) {
    ...
    if (!otpRecord || !otpRecord.verificationId || otpRecord.expiresAt < new Date()) {
      throw new UnauthorizedException('Invalid or expired OTP');
    }

    // Validate verificationId if provided in request
    if (verifyOtpDto.verificationId && verifyOtpDto.verificationId !== otpRecord.verificationId) {
  ...
  async requestRegisterOtp(dto: LoginDto) {
    ...
    const { verificationId, message } = await this.sesService.sendOtp(dto.email);
  ...
  async verifyRegisterOtp(dto: VerifyOtpDto) {
    ...
    if (!record || record.usedAt || record.verifiedAt || !record.verificationId || record.expiresAt < new Date()) {
      throw new UnauthorizedException('Invalid or expired OTP');
    }
  ...
  async aspirantSendLoginOtp(dto: AspirantSendOtpDto) {
    ...
    if (!user || user.isSelfDeleted) {
      throw new NotFoundException('No aspirant account found with this mobile number');
    }
    if (user.role !== 'aspirant') {
      throw new ForbiddenException('This mobile number is not registered as an aspirant');
    }
    if (user.isBlocked) {
      throw new ForbiddenException('Your account has been blocked. Please contact support.');
    }

    const { verificationId, message } = await this.messageCentralService.sendOtp(dto.mobileNumber);
  ...
  async profile(userId: number) {
    ...
    if (user.role === 'aspirant') {
      try {
        ...
        if (aspirant) {
              ...
              if (aspirant.constituencyId) {
                try {
                  if (election.type === 'lok_sabha') {
                    const pc = await this.parliamentaryService.findOne(aspirant.constituencyId);
                    result.constituencyName = pc.name;
                  } else if (election.type === 'state_assembly') {
                    const ac = await this.assemblyService.findOne(aspirant.constituencyId);
                    result.constituencyName = ac.name;
                  } else if (election.type === 'municipal_corporation') {
                    const ward = await this.wardsService.findOne(aspirant.constituencyId);
                    result.constituencyName = `${ward.number} - ${ward.name}`;
                  } else if (election.type === 'gram_panchayat') {
                    const village = await this.gramaPanchayatService.findBySrNo(aspirant.constituencyId);
          ...
          if (aspirant.wardId) {
            try {
              const aspirantWard = await this.wardsService.findOne(aspirant.wardId);
              if (aspirantWard) {
    ...
    try {
      result.hasVoted = await this.votesService.hasUserVotedInActiveWindow(user.id);
    } catch (e) {
  ...
  async registerVoter(dto: RegisterVoterDto) {
    ...
    if (domain !== 'gmail.com') {
      throw new BadRequestException('Only Gmail accounts are allowed for registration');
    }

    // Check email uniqueness
    const existingByEmail = await this.usersService.findByEmail(email);
    if (existingByEmail) {
      const canReRegister = existingByEmail.isSelfDeleted || (existingByEmail.isBlocked && existingByEmail.name === 'Deleted User');
      if (canReRegister) {
        ...
        if (!reactivated) {
          throw new BadRequestException('Failed to reactivate account. Please try again.');
        }
      ...
      if (existingByEmail.isBlocked) {
        throw new ForbiddenException('Your account has been blocked by an admin. Please contact support.');
      }
  ...
  async appleLogin(appleLoginDto: AppleLoginDto) {
    ...
    const decodedToken = await this.firebaseService.verifyIdToken(appleLoginDto.idToken);
  ...
  async googleRegister(googleRegisterDto: GoogleRegisterDto, profilePicture?: Express.Multer.File) {
    // Verify Firebase token
    const decodedToken = await this.firebaseService.verifyIdToken(googleRegisterDto.idToken);
    ...
    if (existingUser) {
      const canReRegister = existingUser.isSelfDeleted || (existingUser.isBlocked && existingUser.name === 'Deleted User');
      if (!canReRegister) {
    ...
    if (!googleRegisterDto.confirm) {
      return {
        message: 'Voter details fetched. Please confirm to proceed with registration.',
        voterDetails: voter,
      };
    }
    ...
    const existingByEpic = await this.usersService.findByEpic(voter?.voter_epic || googleRegisterDto.epicNumber);
    const canReRegister = existingUser && (existingUser.isSelfDeleted || (existingUser.isBlocked && existingUser.name === 'Deleted User'));
    if (existingByEpic && !(canReRegister && existingByEpic.id === existingUser!.id)) {
      throw new ConflictException('This EPIC ID is already registered. Please log in instead.');
    }

    // Reactivate blocked account or create new user
    let user: User;
    if (canReRegister) {
      const reactivated = await this.usersService.reactivateAccount(email, userData);
      user = reactivated!;
    } else {
    ...
    if (profilePicture) {
      const profilePictureUrl = await this.s3Service.uploadFile(profilePicture, `profiles/${user.id}`);
      user.profilePicture = profilePictureUrl;
      await this.usersService.updateUser(user.id, { profilePicture: profilePictureUrl } as any);
    }
