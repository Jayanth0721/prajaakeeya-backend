...
export class WardsService {
  constructor(
    ...
    @InjectRepository(WardMeeting) private readonly meetingRepo: Repository<WardMeeting>
  ) {}
  ...
  findByMunicipality(municipality: string) {
    return this.repo.find({ where: { municipality }, order: { number: 'ASC' } });
  }
  ...
  async findByName(name: string) {
    ...
    ward = wards.find(w => w.name.trim().toLowerCase() === trimmedName.toLowerCase()) ?? null;
    if (ward) return ward;
  ...
  async listByVoterCount() {
    const rows = await this.repo
      ...
      .addSelect("COUNT(DISTINCT usr.id) FILTER (WHERE usr.role = 'voter')", 'voter_count')
      .addSelect("COUNT(DISTINCT usr.id) FILTER (WHERE usr.role = 'aspirant')", 'aspirant_record_count')
      .addSelect("COUNT(DISTINCT usr.id) FILTER (WHERE usr.role = 'aspirant')", 'user_aspirant_count')
      .addSelect("COUNT(DISTINCT usr.id) FILTER (WHERE usr.role IN ('voter', 'aspirant'))", 'total_count')
      .groupBy('ward.id')
  ...
  async createMeeting(dto: CreateWardMeetingDto, createdById: number): Promise<WardMeeting> {
    // Verify ward exists
  ...
  async getAllMeetings(wardId?: number, isActive?: boolean): Promise<WardMeeting[]> {
  ...
  async updateMeeting(id: number, dto: UpdateWardMeetingDto): Promise<WardMeeting> {
