...
export class WardsController {
  ...
  @Get()
  @ApiOperation({ summary: 'Get wards (optionally filter by state, parliamentary, assembly)' })
  @ApiQuery({ name: 'state', required: false, description: 'State name to filter', example: 'Karnataka' })
  @ApiQuery({ name: 'parliamentary', required: false, description: 'Parliamentary constituency to filter', example: 'Bangalore South' })
  @ApiQuery({ name: 'assembly', required: false, description: 'Assembly constituency to filter', example: 'Jayanagar' })
  ...
  @ApiResponse({ status: 200, description: 'Wards returned successfully ordered by voter count' })
  listByVoters() {
  ...
  @ApiQuery({ name: 'q', required: false, description: 'Search term matching ward name or number' })
  @ApiResponse({ status: 200, description: 'Matching wards returned with id, name, number, category' })
