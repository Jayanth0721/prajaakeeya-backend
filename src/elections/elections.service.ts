...
export class ElectionsService implements OnModuleInit {
  ...
  async onModuleInit() {
    const seeds: { type: ElectionType; name: string }[] = [
      ...
      { type: 'municipal_corporation', name: 'Municipal Corporation (Corporator)' },
    ...
    for (const seed of seeds) {
      ...
      } else if (seed.type === 'municipal_corporation' && existing.scope === 'GBA') {
  ...
  async createElection(dto: CreateElectionDto) {
    const existing = await this.repo.findOne({ where: { type: dto.type as ElectionType } });
    if (existing) throw new ConflictException(`Election type '${dto.type}' already exists`);
    return this.repo.save(this.repo.create({ ...dto, type: dto.type as ElectionType }));
  }

  async updateElection(id: number, dto: UpdateElectionDto) {
    const election = await this.repo.findOne({ where: { id } });
    if (!election) throw new NotFoundException(`Election with id ${id} not found`);
  ...
  async findByType(type: ElectionType) {
    ...
    if (!election) throw new NotFoundException(`Election type '${type}' not found`);
    return election;
  }
  ...
  async getConstituencies(type: ElectionType, filters?: { state?: string; district?: string; taluk?: string; gpName?: string }) {
