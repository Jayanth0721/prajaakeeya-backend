...
export class AspirantDiscussionController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Post a message in aspirant discussion room (aspirants only)' })
  @ApiParam({ name: 'aspirantId', type: 'number', description: 'Aspirant ID', example: 1 })
  ...
  async postMessage(
    ...
    return this.discussionService.createMessage(user.id, user.role, Number(aspirantId), dto);
  }

  @Get('ward/:wardNumber/messages')
  @UseGuards(JwtAuthGuard)
  @Public()
  @ApiOperation({ summary: 'Get all aspirants discussion messages for a ward (voters can read)' })
  @ApiParam({ name: 'wardNumber', type: 'string', description: 'Ward number', example: '42' })
  ...
  @ApiParam({ name: 'messageId', type: 'number', description: 'Message ID', example: 1 })
  ...
  async deleteMessage(
    ...
    return this.discussionService.deleteMessage(Number(messageId), user.id, user.role);
  }
}
