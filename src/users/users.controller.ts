import { Controller, Get, Post, Delete, Body, Param, UseGuards, Req, UseInterceptors, Put, UploadedFile, Query } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiParam, ApiBearerAuth, ApiConsumes, ApiBody, ApiQuery } from '@nestjs/swagger';
...
export class UsersController {
  ...
  async findAllVoters(@Query('page') page?: string, @Query('limit') limit?: string, @Query('search') search?: string) {
  ...
  @ApiOperation({ 
    ...
    description: 'Submit a report for a user/voter. Optionally attach a file (PDF, JPEG, PNG).'
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiResponse({ status: 200, description: 'List of reports submitted by the user' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Permanently delete voter account and all associated data' })
  @ApiResponse({ status: 200, description: 'Account deleted or deactivated' })
  @ApiResponse({ status: 400, description: 'Only voters can delete their account' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiResponse({ status: 201, description: 'Chat interaction tracked successfully' })
  ...
  @ApiResponse({ status: 201, description: 'Meeting interaction tracked successfully' })
  @ApiResponse({ status: 404, description: 'User not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiResponse({ status: 201, description: 'Direct meet interaction tracked successfully' })
  @ApiResponse({ status: 404, description: 'User not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiResponse({ status: 201, description: 'Phone call interaction tracked successfully' })
  @ApiResponse({ status: 404, description: 'User not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
