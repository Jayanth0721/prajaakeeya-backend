...
export class AspirantChatController {
  ...
  postMessage(@CurrentUser() user: any, @Param('aspirantId') aspirantId: string, @Body() dto: CreateAspirantMessageDto) {
  ...
  getMessages(@Param('aspirantId') aspirantId: string, @Query() query: GetAspirantMessagesDto) {
  ...
  @ApiParam({ name: 'aspirantId', type: 'number', description: 'Aspirant ID', example: 1 })
  @ApiParam({ name: 'messageId', type: 'number', description: 'Message ID', example: 10 })
