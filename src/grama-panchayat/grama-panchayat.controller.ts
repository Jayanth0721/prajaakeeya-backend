...
export class GramaPanchayatController {
  ...
  @ApiQuery({ name: 'state', required: true, type: String, example: 'Karnataka' })
  ...
  @ApiQuery({ name: 'state', required: true, type: String, example: 'Karnataka' })
  @ApiQuery({ name: 'district', required: true, type: String, example: 'Bagalkote' })
  @ApiResponse({ status: 200, description: 'List of taluks' })
  getTaluks(@Query('state') state: string, @Query('district') district: string) {
    return this.service.getTaluks(state, district);
  }

  @Get('gps')
  @ApiOperation({ summary: 'Get Gram Panchayats for a state + district + taluk' })
  @ApiQuery({ name: 'state', required: true, type: String, example: 'Karnataka' })
  @ApiQuery({ name: 'district', required: true, type: String, example: 'Bagalkote' })
  @ApiQuery({ name: 'taluk', required: true, type: String, example: 'Badami' })
  @ApiResponse({ status: 200, description: 'List of Gram Panchayats' })
  getGPs(@Query('state') state: string, @Query('district') district: string, @Query('taluk') taluk: string) {
  ...
  @ApiResponse({ status: 200, description: 'List of villages with codes and population' })
  getVillages(
