...
export class VoterRollController {
  ...
  @ApiParam({ name: 'wardId', type: 'number', description: 'Ward ID', example: 1 })
  ...
  @ApiParam({ name: 'epic', type: 'string', description: 'EPIC ID', example: 'ABC1234567' })
  ...
  @ApiResponse({ status: 200, description: 'Ward counts returned successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async wardCounts() {
  ...
  @ApiResponse({ status: 201, description: 'Excel file uploaded and processed successfully' })
  ...
  async uploadWardExcel(@Param('wardId') wardId: number, @UploadedFile() file?: Express.Multer.File) {
    if (!file) {
    ...
    const voters = rows
      .map((row) => {
        ...
        if (!normalized.epicNumber || !normalized.name || !normalized.relativeName) {
          return null;
        }
    ...
    return { wardId: Number(wardId), total: uniqueVoters.length, duplicatesRemoved: voters.length - uniqueVoters.length };
  }
}
